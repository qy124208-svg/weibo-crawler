# -*- coding: utf-8 -*-
"""四组痛点词「刚需证明」分析：词频 + 求助信号 + 负面情绪 + 工具提及"""
...
TOOL_WORDS = ['AI', 'ChatGPT', 'GPT', '豆包', '文心', '讯飞', '大模型', 'AI助手', 'AIGC',
    '携程', '飞猪', '去哪儿', '马蜂窝', '同程', '小红书', '抖音', '高德', '百度地图',
    'AI做攻略', 'AI规划', '智能规划', '智能推荐']
...
def hit(texts, words):
    return {w: sum(1 for t in texts if t and w in t) for w in words}


def read(path, col):
    with open(path, encoding='utf-8-sig', newline='') as f:
        return [row.get(col, '') for row in csv.DictReader(f)]


GROUPS = ['A_做攻略难', 'B_找搭子难', 'C_省钱难', 'D_现有工具不满']
...
for name in GROUPS:
    ...
    for t in comments:
        cc.update(segment(t))
    write_counter(f'{OUT_DIR}/高频词_{name}_帖子.csv', pc)
    write_counter(f'{OUT_DIR}/高频词_{name}_评论.csv', cc)

    # 信号命中
    help_hit = hit(all_texts, HELP_WORDS)
    pain_hit = hit(all_texts, PAIN_WORDS)
    tool_hit = hit(all_texts, TOOL_WORDS)

    # 求助命中帖数 / 负面命中帖数（去重）
    help_posts = sum(1 for t in posts if t and any(w in t for w in HELP_WORDS))
    pain_posts = sum(1 for t in posts if t and any(w in t for w in PAIN_WORDS))

    summary.append({
        '组': name, '帖子': len(posts), '评论': len(comments),
        '求助帖数': help_posts, '求助占比': f'{help_posts/len(posts):.1%}',
        '负面帖数': pain_posts, '负面占比': f'{pain_posts/len(posts):.1%}',
        '帖子top10': [w for w, _ in pc.most_common(10)],
        '评论top10': [w for w, _ in cc.most_common(10)],
    })

    print(f'\n===== [{name}] =====')
    print(f'帖子 {len(posts)} 评论 {len(comments)}')
    print(f'求助帖 {help_posts} ({help_posts/len(posts):.1%})  负面帖 {pain_posts} ({pain_posts/len(posts):.1%})')
    print(f'帖子高频: {summary[-1]["帖子top10"]}')
    print(f'评论高频: {summary[-1]["评论top10"]}')
    if name == 'D_现有工具不满':
        top_tools = sorted(tool_hit.items(), key=lambda x: -x[1])[:12]
        print(f'工具提及: {top_tools}')
        summary[-1]['工具提及'] = top_tools

# 写汇总
with open(f'{OUT_DIR}/信号统计.txt', 'w', encoding='utf-8') as f:
    for s in summary:
        f.write(f"[{s['组']}] 帖{s['帖子']} 评论{s['评论']} 求助{s['求助帖数']}({s['求助占比']}) 负面{s['负面帖数']}({s['负面占比']})\n")
        f.write(f"  帖子top10: {s['帖子top10']}\n")
        f.write(f"  评论top10: {s['评论top10']}\n")
        if '工具提及' in s:
            f.write(f"  工具提及: {s['工具提及']}\n")
        f.write('\n')
